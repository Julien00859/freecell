...
def refresh(game):
    ...
    for i, card in zip(range(len(game.freecells)), game.freecells):
        canvas.create_image(
            i * (width_gap + card_width), 0, anchor=tk.NW, image=card_to_img[card]
        )
    ...
    for i, card in zip(range(4, 8), game.foundations):
        if card is placeholder:
            continue
        canvas.create_image(
            2 * width_gap + i * (width_gap + card_width),
            0,
            anchor=tk.NW,
            image=card_to_img[card],
        )
    ...
    for i, column in enumerate(game.columns):
        for j, card in enumerate(column):
            if card in {placeholder, blank_card}:
                continue
            canvas.create_image(
                width_gap + i * (width_gap + card_width),
                card_height + (j + 1) * height_gap,
                anchor=tk.NW,
                image=card_to_img[card],
            )

def init():
    global root, canvas, card_to_img

    root = tk.Tk()
    assets_path = Path(__file__).parent / ".." / "assets"
    card_to_img = {
        card: tk.PhotoImage(
            file=assets_path / f"{card.symbol.name.lower()}_{card.value}.png"
        )
